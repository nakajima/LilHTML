...
class SelectorTests: XCTestCase {
	...
	func testBasic() throws {
		...
		XCTAssertEqual(4, articleHeaders.count, articleHeaders.map { $0.toHTML() }.joined(separator: "\n"))
	}
	...
	func testClass() throws {
		...
		XCTAssertEqual(1, header.count, header.map { $0.toHTML() }.joined())
		XCTAssertEqual("World", header[0].textContent.trimmingCharacters(in: .whitespacesAndNewlines), header.map { $0.toHTML() }.joined())
	}
}
